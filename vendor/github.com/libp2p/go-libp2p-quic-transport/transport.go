...
//向远程对方发送ping探测
func (c *connManager) Ping(network string, timeout time.Duration,raddr *net.UDPAddr) error {
	reuse, err := c.getReuse(network)
	if err != nil {
		return err
	}
	return reuse.Ping(network,timeout, raddr)
}
...
//向对方发送ping
func (t *transport) Ping(raddr ma.Multiaddr, timeout time.Duration) error {
	...
	return t.connManager.Ping(network,timeout, addr)
}
