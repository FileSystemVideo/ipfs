...
// 向对方发送ping探测
func (h *BasicHost) Ping(pi peer.AddrInfo, timeout time.Duration) error {
	...
	resolved, err := h.resolveAddrs(context.Background(), h.Peerstore().PeerInfo(pi.ID))
	if err != nil {
		return err
	}
	h.Peerstore().AddAddrs(pi.ID, resolved, peerstore.TempAddrTTL)
	
	return h.Network().Ping(pi.ID, timeout)
}
...
func (h *BasicHost) GetNatStatus() network.Reachability{
	return h.natStatus
}

func (h *BasicHost) SetNatStatus(status network.Reachability){
	h.natStatus = status
}
...
func dedupAddrs(addrs []ma.Multiaddr) (uniqueAddrs []ma.Multiaddr) {
	...
	for _, addr := range addrs {
		...
		//regStr := `^/ip(4|6)/(10\.|169\.254\.|127\.0\.0\.1|(172\.(1[6-9]|2[0-9]|3[0-1])))`

		//只屏蔽127.0.0.1和 169.254
		regStr := `^/ip(4|6)/(169\.254\.|127\.0\.0\.1)`
		if match, _ := regexp.MatchString(regStr, addr.String()); match {
			continue
		}
		if exists[k] {
			continue
